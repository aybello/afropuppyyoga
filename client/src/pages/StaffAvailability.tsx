...
import { ArrowLeft, Calendar, Megaphone, Plus, Trash2, UsersRound, X } from "lucide-react";
...
export default function StaffAvailabilityPage() {
  ...
  const addLeave = trpc.staffAvailability.addLeave.useMutation({
    onSuccess: () => { refetch(); toast.success("Leave added"); setShowLeaveModal(false); },
  });
  const deleteLeave = trpc.staffAvailability.deleteLeave.useMutation({
    onSuccess: () => { refetch(); toast.success("Leave removed"); },
  });
  const createTeamMember = trpc.staffAvailability.createTeamMember.useMutation({
    onSuccess: () => {
      refetch(); toast.success("Team member added to the org chart"); setShowAddMember(false);
  ...
  const removeTeamMember = trpc.staffAvailability.removeTeamMember.useMutation({
    onSuccess: () => { refetch(); toast.success("Team member removed from the org chart"); setShowLeaveModal(false); setSelectedStaff(null); },
