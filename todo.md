...
- Fixed the live breeder-confirmation failure. Luma returned HTTP 400 because the second registration-question payload, the APY keepsake dropdown, is not accepted by its API. Public class creation now sends only the required **How did you hear about us?** multi-select and the required signed **Terms and Conditions** waiver. The other event data, paid tickets, Luma verification, and no-outreach-before-verification safety checks remain intact.
