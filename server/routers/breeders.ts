...
const eventBlockSchema = z.object({
  ...
  classSlot: z.enum(["10:00", "11:30", "13:30"]).optional(),
  ...
  if (!event.isPrivateEvent && !event.classSlot) {
    context.addIssue({ code: "custom", path: ["classSlot"], message: "Choose the APY public class slot for this studio event." });
  }
});
...
export function generateConfirmationEmail(opts: {
